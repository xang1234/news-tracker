...
uv run news-tracker eval extraction --verbose            # Score rule extractor recall/precision vs golden set
uv run news-tracker eval extraction --method llm --verbose  # Score the LLM extraction pass (needs SCORING_OPENAI_API_KEY)
...
| Themes | `src/themes/` | `ThemeRepository`, `LifecycleClassifier`, `VolumeMetricsService`, `ThemeRankingService`, `AttributionService` (doc→metric attribution, pure `attribute_documents`) |
...
- **Extraction Eval Harness**: `src/eval/` (epic `7th.1`) — gates the hybrid-LLM extraction rollout. `evaluate(golden_docs, extractor)` scores any `extractor: GoldenDocument → list[EvidenceClaim]` against a checked-in golden set by matching normalized `(subject, predicate, object)` triples → recall/precision/f1 (TP = expected & extracted, FN = missed → recall, FP = spurious → precision). `rule_extractor` adapts the current `extract_claims_from_document`; the golden set (`golden/extraction_golden.json`) deliberately includes implicit claims the regex misses (current: recall 0.70 / precision 1.00) so the LLM pass (`7th.2`) has measurable recall headroom. CI gate in `tests/test_eval/test_golden_eval.py` (recall ≥ 0.6, precision ≥ 0.9 floors); ad-hoc via `news-tracker eval extraction [--method rule|llm] [--verbose]` (`make_llm_extractor` bridges the async LLM pass to the sync harness for offline, real-key scoring). The same harness scores the LLM/hybrid extractor — the gate is "beat regex recall without dropping precision below the floor."
- **LLM Claim Extraction Pass**: `src/claims/llm_extractor.py` (epic `7th.2`/`7th.3`/`7th.4`) — the second extraction pass. `LLMClaimExtractor.extract(doc_id, content)` reads raw document text and emits `EvidenceClaim`s with `extraction_method="llm"`, catching implicit/multi-clause relationships the regex `narrative_extractor` misses (e.g. "demand from NVIDIA for TSMC's packaging" ⇒ `TSMC supplies_to NVIDIA`). Pure prompt + parser in `llm_prompt.py` mirror `briefing`/`qa`: `build_extraction_prompt` lists the controlled `NARRATIVE_PREDICATES` vocabulary (promoted from `narrative_extractor` as the single source of truth); `parse_extraction_response` is None-safe, normalizes predicates to snake_case, drops out-of-vocabulary/low-confidence/duplicate entries, and — crucially — builds claims with the **same `make_claim_key(lane, doc_id, subject, predicate, object)`** the rule pass uses, so identical triples dedup-merge regardless of which pass found them. LLM round-trip is the shared `JsonLLMClient` (see **Grounded-LLM Substrate**), injected for tests; degrades to `[]` on no-key/open-breaker/unparseable so a caller can fall back to the rule pass. **`7th.3` integration**: wired into `processing_service` Stage 6 behind `llm_claim_extraction_enabled` (requires `narrative_claim_extraction_enabled`). The pure `is_high_value(doc, config)` gate bounds cost — the paid pass runs only on docs clearing `min_authority` (or an opt-in `min_engagement`); `merge.merge_claims(rule_claims, llm_claims)` (pure) then unions by `claim_key`, keeping a triple found by both passes once as the **richer rule claim re-stamped `extraction_method="hybrid"`** (via `dataclasses.replace`, no input mutation). LLM augmentation is phase-resilient (an LLM failure never drops the rule claims). **`7th.4` review hold**: `_persist_claim` still upserts every claim, but `triggers.check_low_confidence_llm` (pure; trusts `rule`/`hybrid`, holds only pure-`llm` claims with confidence below `review_confidence`) routes speculative claims into the `news_intel.review_tasks` queue (`claim_review`/`low_confidence`) and **skips reconciliation** for them — so they can't feed assertions until approved. **`lrb` approval side**: `ReviewResolutionService.resolve(task_id, resolution)` (`src/claims/review_resolution.py`), for an *approved held* task (`was_held_for_review`: `claim_review`/`low_confidence`), **reconciles its claims first and only then transitions the task to `resolved`** — so a reconciliation failure leaves the task un-transitioned (never an "approved" task whose claims never reached assertions; `resolved` is terminal, so a post-transition revert isn't possible). Other resolutions/task types skip reconciliation. (No API/CLI surface for review-task resolution yet — follow-up.) Tuning via `LLMExtractionConfig` (`LLM_EXTRACTION_*`: `max_claims`, `min_confidence`, `review_confidence` (> `min_confidence`, validator-enforced), `min_authority`, `min_engagement`); offline scoring via `news-tracker eval extraction --method llm`.
...
- **Doc→Metric Attribution**: `src/themes/attribution.py` (epic `o59.1`) — explainability via an *exact additive decomposition* of the displayed aggregate, not a heuristic. Each doc's `sentiment_contribution = wᵢ·sᵢ/Σw` (`sᵢ∈{+1,0,−1}` from the label) **sums to `bullish_ratio − bearish_ratio`**, and `volume_contribution = wᵢ/Σw` sums to 1. Reuses the aggregator's own `SentimentAggregator.document_weight` (made public for this) so contributions reconcile with the number on screen; pure `attribute_documents(docs, *, weigher, reference_time, limit)` ranks by |sentiment contribution|. `AttributionService.attribute_theme` fetches via the shared `document_sentiments_from_rows` mapper. Surfaced at `GET /themes/{id}/attribution`; consumed server-side by `o59.2` (alert evidence).
- **Alert Evidence Enrichment**: `src/alerts/evidence.py` + `AlertService._enrich_evidence` (epic `o59.2`) — every theme-metric alert carries the documents that caused it (the "receipt"). After dedup/rate-limit filtering (so only persisted alerts are enriched), `AlertService` runs the injected `AttributionService` for the alert's theme and attaches the top-`evidence_top_k` contributing docs (over `evidence_window_days`) as `Alert.supporting_evidence` (JSONB, migration 046). Only triggers whose cause decomposes into documents are enriched (`evidence.EVIDENCE_TRIGGER_TYPES` = sentiment_velocity / extreme_sentiment / volume_surge); lifecycle/new-theme/propagated alerts already carry their cause in `trigger_data`. Best-effort: any attribution failure leaves the alert un-enriched, never blocking it; no-op when no attribution service is injected (only `daily_job` wires it). Pure `document_evidence_payload` builds the receipt; surfaced via `/alerts` (`AlertItem.supporting_evidence`). Tuning: `ALERTS_EVIDENCE_WINDOW_DAYS` (7), `ALERTS_EVIDENCE_TOP_K` (5).
...
| GET | /themes/{id}/attribution | Ranked documents that moved the theme's sentiment/volume window (epic `o59.1`) |
