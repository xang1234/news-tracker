...
- **Claim Reconciliation (contradiction tiers)**: pluggable tiers each expose `reconcile_claim(claim) -> ResolvedAssertion | None` and no-op for claims they don't apply to; they feed `aggregate_assertion`, flipping an assertion to `disputed`. (1) **Numeric** — `src/assertions/numeric_contradiction.py` (`numeric_link_types`/`classify_numeric_links`: group comparable facts by `(metric,period,unit)`, anchor per group) + `NumericReconciler` (`numeric_reconciler.py`); comparable facts via `ClaimRepository.list_comparable_numeric_claims` (migration 044 index). (2) **Predicate-polarity** — `src/assertions/predicate_contradiction.py` (`ANTONYM_PREDICATES`, `validity_overlaps`, `classify_polarity_links`: antonym predicates like `expands_capacity`↔`constrains_capacity` on the same subject with overlapping validity) + `PredicateContradictionReconciler` (`predicate_reconciler.py`); comparable claims via `ClaimRepository.list_claims_by_subject_predicates`. Shared subject grounding: `resolve_claim_subject` (`claim_reconciliation.py`, reuses `EntityResolver`). Wired into `processing_service` Stage 6 (`_persist_claim`: resolve-before-upsert, then run each tier in `self._claim_reconcilers`), gated by `claim_reconciliation_enabled` (requires `narrative_claim_extraction_enabled`).
