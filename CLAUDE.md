...
- **Numeric Contradiction Reconciliation**: `src/assertions/numeric_contradiction.py` (`numeric_link_types`/`classify_numeric_links`: group comparable facts by `(metric,period,unit)`, pick a per-group anchor, emit support/contradiction links) feeds `aggregate_assertion`, flipping an assertion to `disputed` on numeric disagreement. `src/assertions/numeric_reconciler.py` (`NumericReconciler` + `resolve_numeric_subject`) is the live orchestrator: resolve the numeric claim's subject → concept_id (reuses `EntityResolver`), find comparable facts via `ClaimRepository.list_comparable_numeric_claims` (migration 044 partial index), recompute + persist the assertion and links. Wired into `processing_service` Stage 6 (`_persist_claim`), gated by `numeric_reconciliation_enabled` (requires `narrative_claim_extraction_enabled`).
