...
# Market catalyst models


class MarketCatalystTickerItem(BaseModel):
    """Ticker most directly tied to a live catalyst."""

    ticker: str = Field(..., description="Ticker symbol")
    mention_count: int = Field(..., description="Narrative mention count")


class MarketCatalystRelatedTickerItem(BaseModel):
    """Propagated follow-on ticker impact."""

    ticker: str = Field(..., description="Related ticker symbol")
    source_ticker: str = Field(..., description="Primary ticker driving the propagation")
    impact: float = Field(..., description="Signed propagated impact")
    depth: int = Field(..., ge=1, description="Hops from the primary ticker")
    relation: str = Field(..., description="Graph edge relation behind the impact")


class MarketCatalystEvidenceItem(BaseModel):
    """Supporting document behind a catalyst."""

    document_id: str = Field(..., description="Supporting document identifier")
    platform: str | None = Field(default=None, description="Source platform")
...
class MarketCatalystItem(BaseModel):
    """Actionable stock-market catalyst synthesized from live narratives."""

    run_id: str = Field(..., description="Narrative run identifier")
    theme_id: str = Field(..., description="Parent theme identifier")
    theme_name: str = Field(..., description="Human-readable theme label")
    ...
    related_tickers: list[MarketCatalystRelatedTickerItem] = Field(
        default_factory=list, description="Follow-on tickers from graph propagation"
    )
    evidence: list[MarketCatalystEvidenceItem] = Field(
        default_factory=list, description="Supporting documents"
    )
    started_at: str = Field(..., description="Narrative run start timestamp")
    last_document_at: str = Field(..., description="Most recent document timestamp")


class MarketCatalystsResponse(BaseModel):
    """Ranked list of market catalysts."""

    catalysts: list[MarketCatalystItem] = Field(..., description="Catalysts sorted by impact score")
    total: int = Field(..., description="Number of catalysts returned")
    latency_ms: float = Field(..., description="Processing latency in milliseconds")
