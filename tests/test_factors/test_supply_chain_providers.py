...
async def test_eia_provider_parses_monthly_values_missing_values_and_revisions() -> None:
    ...
    assert observations[0].available_at == datetime(2026, 6, 14, tzinfo=UTC)
    ...
    assert observations[0].metadata["estimated_release_at"] == "2026-06-14T00:00:00+00:00"
...
async def test_census_provider_parses_monthly_hs_rows() -> None:
    ...
    assert observations[0].observation_date == date(2026, 4, 1)
    assert observations[0].available_at == datetime(2026, 6, 4, tzinfo=UTC)
    assert observations[0].metadata["estimated_release_at"] == "2026-06-04T00:00:00+00:00"
